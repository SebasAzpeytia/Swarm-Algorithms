...
class Particle:
    ...
    def move(self, c1: float, c2: float, g: PVector):
        ...
        self.Vi = K * (self.Vi + cognitive + social)
        self.X_actual += self.Vi

class Population:
    def __init__(self, sizePop=100, rangePositions=(0, 10)):
        self.globalPos = PVector(range=rangePositions)
        self.particles = [Particle(range_positions=rangePositions) for _ in range(sizePop)]
    ...
    def evol(self, c1, c2):
        ...
        self.globalPos = min((P.X_best for P in self.particles), key=self.fitnessFunction)
    
    def fitnessFunction(self, vector: PVector):
        # Función de costo: minimizar x^2 + y^2
        return (math.pow(vector.x, 2) + math.pow(vector.y, 2))

Pop = Population(rangePositions=(-100, 100), sizePop=50)
